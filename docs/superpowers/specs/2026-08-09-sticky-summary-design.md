...
上の本文は設計時の記述で、実装中に見つかった欠陥を受けて挙動が変わった箇所がある。コードが正で、以下が差分。

**サニタイズの適用範囲が広がった（§3）。** 当初はタイトルだけを想定していたが、sticky に到達する文字列は**すべて**シリアライズ形式への入力である、という不変条件に一般化した。`renderSticky` は 3 段のヘルパを持つ。

- `sanitizeInline` — 空白畳み込み + `&`・`<`・`>` の実体参照化。バックティックは潰さない
- `sanitizePath` — `sanitizeInline` + バックティック潰し。コードスパンで囲む値（ファイルパス）用
- `sanitizeFenced` — `<` の実体参照化と ``` の無効化のみ。改行は情報なので保つ。フェンス内のエラー本文用

適用先は 5 経路：指摘タイトル、`droppedFiles`、`oversizedFiles`、`failure.message`、スレッドのファイルパス。最後の 1 つは最終レビューの再レビューまで漏れていた。

**`decideEvent` が「生きている判定」を見るようになった（§6）。** 未解決の指摘が残っているだけでは `REQUEST_CHANGES` を再提出しない。GitHub は前回の判定を生かし続けるので、同じ判定を出し直しても状態は変わらず、コメント 0 件の Review と通知が増えるだけだった。`APPROVE` 側も同様。加えて、差分外の指摘を破棄した実行では `APPROVE` を出さない。

**`dismissOwnApproval` は `getOwnVerdict` + `dismissReview` に分かれた（§4 / §6）。** 判定の照会と取り下げで走査を共有するため。**`DISMISSED` に当たったら走査を打ち切って `null` を返す**。読み飛ばして古い判定を掘り出すと、GitHub がもう有効としていない `CHANGES_REQUESTED` を「生きている」と誤認し、未解決の critical を抱えたまま PR がマージ可能になる。

**投稿後のスレッド再取得は条件付きで、失敗しても続行する（§5 ステップ 12）。** 新しいコメントを投稿したときだけ取り直す。投稿していなければ既存の取得結果が最新であり、ここで叩くと API の一時失敗が「投稿済みのレビュー」を失敗扱いに変えてしまう（エージェントの課金は済んでいる）。取得に失敗した場合も既存の結果で描画して成功を維持する。

**エラー処理表への追加（§8）。**

| 事象 | 挙動 |
| --- | --- |
| 差分外のファイルへの指摘 | 破棄し、件数とファイル名を sticky のバナーに出す |
| `GET /user` が 403 / 404 以外で失敗 | 例外を投げ直し、`findSticky` の失敗として扱う。ここで Bot 判定にフォールバックすると、PAT 運用で sticky を取り違えて二重に作る |

## 14. マージ後の follow-up

最終レビューで triage し、このブランチには入れなかったもの。

1. **`tests/io/github.test.ts` が存在しない。** `findSticky` の作成者判定と `getOwnVerdict` の 4 分岐は、論理の取り違えが「sticky の二重作成」や「誤った判定」に直結する。最終レビューが最も価値の高い follow-up と評価した
2. **sticky の本文サイズに上限が無い。** `run` マーカーは実行のたびに積み上がる（1 行 150 バイト程度）。GitHub の 65,536 文字上限を超えると `upsertSticky` が 422 を返し、その失敗は握られるので `reviewed` が凍結し、以後**無言で**毎回フルレビューに課金され続ける。履歴を直近 N 件に丸めるか、解決済みリストを切り詰める必要がある
3. **`model` input が生のままマーカーに入る。** `config.ts` の自由入力なので、`-->` を含む値でマーカーを途中で閉じられる。ワークフロー作成者の自傷にしかならないが、他の値と非対称
4. **トークン種別を変えると sticky が孤児になる。** `GITHUB_TOKEN` ↔ PAT を切り替えると作成者判定が変わり、既存の sticky が一致しなくなって二重に作られる
5. **`analysis.excludedFiles` が消費者を失っている。** 旧サマリーの削除で行き場がなくなった。除外が起きたことをユーザーが知る手段が無いので、sticky に出すのが筋
6. **差分サイズ上限で落ちたファイルへの指摘が「差分外」として報告される。** 実際は PR に含まれるファイルなので、2 行上の oversized バナーと食い違って見える
